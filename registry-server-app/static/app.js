(() => {
  ...
  function docHtml(d){const ex=expiryInfo(d.expiry_date),meta=[];if(d.issue_date)meta.push(`<div><span class="meta-label">Выдан:</span>${esc(dateRu(d.issue_date))}</div>`);if(d.expiry_date)meta.push(`<div><span class="meta-label">Срок:</span><span class="${ex.cls}">${esc(ex.text)}</span></div>`);if(d.status)meta.push(`<div><span class="meta-label">Статус:</span>${esc(d.status)}</div>`);if(d.category)meta.push(`<div><span class="meta-label">Категория:</span>${esc(d.category)}</div>`);if(d.source==='spk2'&&d.issuer)meta.push(`<div><span class="meta-label">Кем выдано:</span>${esc(d.issuer)}</div>`);const url=safeUrl(d.url);const link=url?`<a class="registry-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">${d.url_kind==='direct'?'Открыть карточку в реестре ↗':'Открыть реестр ↗'}</a>`:'';return `<div class="doc"><div class="doc-number">${esc(d.number||'Документ без номера')}</div>${d.organization?`<div class="doc-org">${esc(d.organization)}</div>`:''}${meta.length?`<div class="doc-meta">${meta.join('')}</div>`:''}${d.activity?`<div class="activity">${esc(d.activity)}</div>`:''}${link}</div>`;}
